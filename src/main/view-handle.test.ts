...
//
// Faithful to the real host's semantics so the compositor's behavior
...
describe('applyPlacement after dispose — idempotent late IPC (the (b) risk)', () => {
	it('drops a place frame that arrives after dispose (no setBounds, no mount)', async () => {
		...
		const boundsBefore = nativeView.bounds.length
		// A `place` IPC arriving AFTER teardown must be dropped entirely — and the
		// sink reports the drop (a caller doing bookkeeping alongside it, e.g. the
		// deck-app's keepAlive LRU, must not treat this as an applied frame).
		expect(
...
describe('createViewHandle — moveTo (cross-window, moveTo cross-window migration)', () => {
	...
	it('moves the view from src host to dest host (no throw) and re-binds to dest', async () => {
		...
		// Both are ACCEPTED frames (post-migration, not migrating) → return true.
		expect(
			handle.applyPlacement({ visible: true, bounds: { x: 1, y: 2, width: 3, height: 4 } }),
		).toBe(true)
		expect(destHost.ids()).toContain('v1')
		expect(handle.applyPlacement({ visible: false })).toBe(true)
		expect(destHost.ids()).not.toContain('v1')
		expect(srcHost.ids()).not.toContain('v1')
	})

	// ───────────────────────────────────────────────────────────────────────────
	// A `place` frame that lands WHILE a moveTo is mid-flight is DROPPED (see the
	// `migrating` guard) — the sink must report that via a `false` return so a
	// caller doing bookkeeping alongside it (deck-app's keepAlive LRU) never
	// treats a frame the sink itself discarded as applied.
	//
	// `withLock`'s microtask scheduling makes the window deterministic (not a
	// race): once `moveTo` starts, the FIRST microtask tick runs its native
	// migration to completion (no internal await without `rehome`) and only
	// THEN suspends on `await doMove(...)` — `migrating` flips back to false one
	// microtask AFTER that. A single `await Promise.resolve()` right after
	// kicking off `moveTo` lands exactly inside that one-tick window.
	// ───────────────────────────────────────────────────────────────────────────
	it('applyPlacement mid-moveTo is dropped and returns false', async () => {
		const { handle, nativeView } = placeInSrc('v1')
		const { dest } = makeDest()

		const movePromise = handle.moveTo(dest, { zone: 0 })
