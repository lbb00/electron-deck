 ...
 * dock-react, per check-browser-purity.js) builds with platform 'browser'.
 */
...
const HELP = `Usage: node scripts/check-bundle-size.js [--update] [--baseline <file>]
 ...
 * browser-consumed surface (per check-browser-purity.js) never does. */
...
async function measureEntry(subpath, distRelPath, workDir) {
	...
	const entryFile = join(workDir, subpath.replace(/[^a-zA-Z0-9]+/g, '_') + '.entry.js')
