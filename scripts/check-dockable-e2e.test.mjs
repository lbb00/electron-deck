...
const wrapper = join(repo, 'scripts/check-dockable-e2e.mjs')
...
async function writeFixture({ pathWithSpaces = false, shortTimeout = false, electronSource }) {
	...
	await Promise.all([
		writeFile(join(fixture, 'scripts/check-dockable-e2e.mjs'), shortTimeout ? script.replace('}, 60_000)', '}, 500)') : script),
		writeFile(join(fixture, 'examples/dockable-demo/main.mjs'), ''),
...
`,
	...
	const { child, getOutput } = runNode(join(fixture, 'scripts/check-dockable-e2e.mjs'), {
		cwd: fixture,
		env: { DECK_E2E_OUTPUT_DIR: output, EXPECTED_MAIN: await realpath(join(fixture, 'examples/dockable-demo/main.mjs')) },
	})
	try {
...
`,
	...
	const wrapperPath = join(fixture, 'scripts/check-dockable-e2e.mjs')
